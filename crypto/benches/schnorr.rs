...
use goldilocks_crypto::{sign, verify_signature, Point, ScalarField};
...
fn bench_schnorr_sign(c: &mut Criterion) {
    ...
    group.bench_function("sign_auto_nonce", |b| {
        b.iter(|| sign(black_box(&private_key_bytes), black_box(&message)))
    });
...
fn bench_point_operations(c: &mut Criterion) {
    ...
    group.bench_function("point_double", |b| b.iter(|| black_box(&point).double()));
