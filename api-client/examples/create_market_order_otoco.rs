...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "═".repeat(80));
    println!("🚀 CREATE MARKET ORDER WITH OTOCO (Entry + Stop Loss + Take Profit)");
    println!("{}", "═".repeat(80));
    println!();
    ...
    let entry_price = 899030;      // $55,000 - Market order will execute at current market price
    let stop_loss_price = 880000;  // $50,000 - Stop loss trigger
    let take_profit_price = 920000; // $60,000 - Take profit trigger

    // Calculate order expiry: 30 days from now (in milliseconds)
    let now = SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis() as i64;
    let order_expiry = now + (30 * 24 * 60 * 60 * 1000); // 30 days

    println!("📝 Creating Market Order with OTOCO (One Triggers A One Cancels The Other)");
    println!("  Entry: Market Buy at ~${}", entry_price / 100);
    ...
    // Order 0: Market entry order (parent)
    ...
    let request = CreateGroupedOrdersRequest {
        ...
        orders: vec![
            ...
            CreateOrderRequest {
                ...
                base_amount: 1000,  // 0.001 BTC (in smallest unit)
                price: entry_price,    // Max price for market order
            ...
            // Order 1: Stop Loss (child, position-tied)
            // Note: StopLossOrder requires TimeInForce = ImmediateOrCancel (0)
            // Note: Market index must match parent order (all orders must have same market index)
            CreateOrderRequest {
                account_index,
                order_book_index: 1,  // Must match parent order's market index
                client_order_index: 0,  // Must be 0 for grouped orders
                ...
                time_in_force: 0,      // ImmediateOrCancel (required for StopLossOrder)
                reduce_only: true,     // Only reduce position
                trigger_price: stop_loss_price,
            },
            // Order 2: Take Profit (child, position-tied)
            // Note: TakeProfitOrder requires TimeInForce = ImmediateOrCancel (0)
            // Note: Market index must match parent order (all orders must have same market index)
            CreateOrderRequest {
                account_index,
                order_book_index: 1,  // Must match parent order's market index
                ...
                time_in_force: 0,      // ImmediateOrCancel (required for TakeProfitOrder)
                reduce_only: true,     // Only reduce position
    ...
    println!("✅ Market Order with OTOCO submitted!");
    println!("📥 Response:");
    ...
    if code == 200 {
        ...
        println!("\n📊 How OTOCO Works:");
        println!("  1. Market order executes at current price");
        println!("  2. Both Stop Loss and Take Profit are automatically placed");
        println!("  3. If price drops to ${} → Stop Loss executes, Take Profit cancelled", stop_loss_price / 100);
        println!("  4. If price rises to ${} → Take Profit executes, Stop Loss cancelled", take_profit_price / 100);
