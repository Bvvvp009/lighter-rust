...
impl ScalarField {
    ...
    pub fn equals(&self, rhs: &ScalarField) -> bool {
        self.to_bytes_le().ct_eq(&rhs.to_bytes_le()).into()
    }
    ...
    pub fn sub_inner(&self, a: &ScalarField) -> (ScalarField, u64) {
        ...
        for (i, slot) in r.iter_mut().enumerate() {
            ...
            let z1_hi = 0u64.wrapping_sub(borrow1 as u64);
        ...
        (ScalarField(r), 0u64.wrapping_sub(c & 1))
    }
    ...
    /// Montgomery multiplication (REDC algorithm, 5-limb, 320-bit).
    ///
    /// Computes `self * rhs * R⁻¹ mod N` where `R = 2^320`.  This is a
    /// low-level primitive; prefer [`mul`](Self::mul) for ordinary scalar
    /// arithmetic.
    ///
    /// # Carry analysis
    ///
    /// In the inner loop two independent 64-bit carries `cc1` and `cc2` are
    /// accumulated.  After processing all 5 columns the final limb is set to
    /// `r[4] = cc1.wrapping_add(cc2)`.  Because each partial product is at most
    /// `(2^64-1)² + (2^64-1) + (2^64-1) < 2^128`, both `cc1` and `cc2` are
    /// bounded by `2^64-1`, so their wrapping sum cannot exceed `2^64-2`; no
    /// real overflow can occur.  A conditional subtraction of `N` follows to
    ...
    /// Multiplies two **canonical** scalars and returns a **canonical** result.
    ///
    /// Unlike [`mul`](Self::mul) — which round-trips through Montgomery form and
    /// may require an extra `to_canonical()` call inside certain call sites —
    /// this function uses `BigUint` arithmetic and always returns a value in
    /// `[0, N)` without any post-processing.
    ///
    /// Use `mul_canonical` when the result will be passed directly to
    /// `Point::mul()` (which expects a canonical scalar).  For all other scalar
    /// arithmetic `mul` is faster; both produce the same mathematical value.
    ///
    /// # Performance
    ///
    /// `mul_canonical` allocates heap memory via `BigUint` and is noticeably
    /// slower than `mul`.  Prefer `mul` in hot paths and call `to_canonical()`
    ...
    ///
    /// Uses Fermat's little theorem: `a⁻¹ ≡ a^(N-2) mod N` via `BigUint::modpow`.
    /// Returns `None` if `self` is zero (zero has no inverse).
    ///
    /// # ⚠️ Not constant-time
    ///
    /// The underlying `BigUint::modpow` implementation branches on the exponent
    /// bits and is **not** constant-time.  Do **not** call this method with a
    /// **secret** scalar (e.g. a private key or nonce); doing so leaks timing
    /// information to an attacker.  This method is safe for public, non-secret
    /// scalars such as signature components during verification.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// Deserialises 40 little-endian bytes into a scalar **without** checking
    /// that the value is less than the field modulus `N`.
    ///
    /// # ⚠️ Warning — non-canonical inputs accepted
    ///
    /// This function does **not** reduce the value modulo `N`.  Callers that
    /// need to validate untrusted input (e.g. from the network or a file)
    /// **must** use [`from_canonical_bytes_le`](Self::from_canonical_bytes_le)
    /// instead, which rejects encodings ≥ N.  Feeding a non-canonical scalar
    /// into signing or verification produces incorrect results.
    ///
    /// This raw path is provided for internal construction of known-canonical
    /// values (e.g. after arithmetic that is proven to stay in range) and for
    /// low-level tests that deliberately exercise boundary conditions.
    ...
    /// Converts a scalar from little-endian bytes and rejects non-canonical encodings.
    pub fn from_canonical_bytes_le(data: &[u8]) -> Result<Self, String> {
        let scalar = Self::from_bytes_le(data)?;
    ...
    pub fn recode_signed(&self, window_width: usize) -> Vec<i32> {
        ...
        for digit in digits.iter_mut().take(num_digits) {
            ...
            if acc_len < w {
                if j < limbs.len() {
                    ...
                    acc = if shift_amt < 64 { nl >> shift_amt } else { 0 };
                } else {
                    bb = (acc as u32) & mw;
                    acc = 0;
                }
    ...
    pub fn add_raw(&self, val: u64) -> ScalarField {
        let added = ScalarField([
            self.0[0].wrapping_add(val),
            self.0[1],
            self.0[2],
            self.0[3],
            self.0[4],
    ...
    pub fn sample_crypto() -> ScalarField {
        ...
        loop {
            ...
            if random_big != BigUint::from(0u8) && random_big < order_big {
                // Convert to limbs
...
impl fmt::Display for ScalarField {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "ScalarField({:016x}{:016x}{:016x}{:016x}{:016x})",
