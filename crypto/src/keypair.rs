...
use crate::{
    sign, signature::Signature, verify_signature, CryptoError, Point, Result, ScalarField,
};
use zeroize::Zeroize;
...
impl KeyPair {
    ...
    pub fn from_private_key_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let sk = ScalarField::from_canonical_bytes_le(bytes).map_err(|_| {
            CryptoError::NonCanonicalScalar {
                field: "private key",
            }
        })?;
        if sk.is_zero() {
...
mod tests {
    ...
    fn debug_does_not_leak_private_key() {
        ...
        assert!(
            ...
            "private key must be redacted in Debug output"
        );
