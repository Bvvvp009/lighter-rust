...
fn test_schnorr_signature_components() {
    ...
    // Fixed canonical nonce for deterministic testing (40 bytes)
    let mut nonce_bytes = [0u8; 40];
    nonce_bytes[..8].copy_from_slice(&0x0123_4567_89AB_CDEFu64.to_le_bytes());
    ...
    // 1. Compute R = nonce * G
    let nonce_scalar = ScalarField::from_canonical_bytes_le(&nonce_bytes[0..40]).unwrap();
    ...
    let private_scalar = ScalarField::from_canonical_bytes_le(&private_key).unwrap();
...
fn test_schnorr_random_nonce_quality() {
    ...
    let mut private_key = vec![0u8; 40];
    private_key[0] = 1;
    ...
    for i in 0..signatures.len() {
        for j in (i + 1)..signatures.len() {
            assert_ne!(
                signatures[i],
                signatures[j],
                "Signatures {} and {} are identical - RNG might be broken!",
                i + 1,
                j + 1
            );
        }
