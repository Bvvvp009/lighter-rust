...
High-level architecture and design of the Lighter Rust SDK.

## System Architecture

The SDK is organized into five layers, each building on the previous:

```
┌─────────────────────────────────────┐
│        lighter-sdk Layer            │  High-level trading SDK (recommended entry point)
│       (lighter-sdk crate)           │
├─────────────────────────────────────┤
│       API Client Layer              │  HTTP + WebSocket client, order management
│      (api-client crate)             │
├─────────────────────────────────────┤
│         Signer Layer                │  Key management, auth tokens, tx signing
│        (signer crate)               │
├─────────────────────────────────────┤
...
**Dependency direction**: `lighter-sdk` → `api-client` → `signer` → `crypto` → `poseidon-hash`. Lower layers have no knowledge of higher layers.
...
**Purpose**: HTTP and WebSocket client for all Lighter Exchange endpoints

**Responsibilities**:
- REST API calls (orders, accounts, markets, candles, etc.)
...
**Key Types**:
- `LighterClient`: Main client struct (HTTP + signing)
- `CreateOrderRequest`: Order creation payload
- `WsMessage`: WebSocket message enum (12 variants)

**Dependencies**: `signer`, `crypto`, `poseidon-hash`

### 5. lighter-sdk Layer (`lighter-sdk`)

**Purpose**: High-level, ergonomic SDK for typical trading workflows

**Responsibilities**:
- Combines API client + signer into a single easy-to-use interface
- Pre-built flows: place order → monitor → cancel
- Smoke-test and validation helpers

**Dependencies**: `api-client`, `signer`
