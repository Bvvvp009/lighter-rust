...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deadline = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs() as i64 + (7 * 3600);
    ...
    println!("Usage:");
    println!(
        "  Add to HTTP header: Authorization: {}",
        &auth_token[..auth_token.len().min(50)]
    );
    println!(
        "  Or as query param: ?auth={}",
        &auth_token[..auth_token.len().min(50)]
    );
