...
use goldilocks_crypto::{
    batch_verify, sign, sign_with_nonce, validate_public_key, verify_signature, Fp5Element,
    Goldilocks, Point, ScalarField,
};
...
fn sign_rejects_short_private_key() {
    let msg = [0u8; 40];
    assert!(
        sign(&[0u8; 20], &msg).is_err(),
        "20-byte key must be rejected"
    );
}
...
fn sign_rejects_long_private_key() {
    let msg = [0u8; 40];
    assert!(
        sign(&[0u8; 41], &msg).is_err(),
        "41-byte key must be rejected"
    );
}
...
#[test]
fn sign_with_nonce_rejects_short_nonce() {
    let sk = fixed_sk_bytes();
    let msg = [0u8; 40];
    assert!(
        sign_with_nonce(&sk, &msg, &[0xAA; 8]).is_err(),
        "short nonce must be rejected instead of being silently padded"
    );
}

#[test]
fn sign_with_nonce_rejects_long_nonce() {
    let sk = fixed_sk_bytes();
    let msg = [0u8; 40];
    assert!(
        sign_with_nonce(&sk, &msg, &[0xBB; 41]).is_err(),
        "long nonce must be rejected instead of being silently truncated"
    );
}

#[test]
fn sign_rejects_non_canonical_private_key_scalar() {
    let msg = [0u8; 40];
    let non_canonical = ScalarField::N.to_bytes_le();
    assert!(
        sign(&non_canonical, &msg).is_err(),
        "private keys encoded with scalar modulus N must be rejected"
    );
}

#[test]
fn verify_rejects_non_canonical_signature_scalars() {
    let (_, pk) = random_keypair();
    let msg = [0u8; 40];
    let mut sig = [0u8; 80];
    sig[40..].copy_from_slice(&ScalarField::N.to_bytes_le());
    assert!(
        verify_signature(&sig, &msg, &pk).is_err(),
        "signature components encoded with scalar modulus N must be rejected"
    );
}

#[test]
fn verify_rejects_neutral_public_key_forgery() {
    let public_key = [0u8; 40];
    let message = [0u8; 40];
    ...
    let challenge = hash_to_quintic_extension(&pre_image);
    let mut signature = [0u8; 80];
    signature[40..].copy_from_slice(&ScalarField::from_fp5_element(&challenge).to_bytes_le());

    assert!(
        verify_signature(&signature, &message, &public_key).is_err(),
        "neutral public keys must be rejected inside verification, not only by validation helpers"
    );
}
...
fn scalar_fresh_sample_is_canonical() {
    ...
    for _ in 0..20 {
        ...
        assert!(
            s.is_canonical(),
            "sample_crypto must always return a canonical scalar"
        );
    }
}
...
fn scalar_hex_roundtrip() {
    ...
    assert_eq!(
        original.to_bytes_le(),
        restored.to_bytes_le(),
...
fn scalar_hex_roundtrip_with_0x_prefix() {
    ...
    assert_eq!(
        original.to_bytes_le(),
        restored.to_bytes_le(),
        "0x-prefixed hex round-trip failed"
    );
}
...
fn scalar_from_hex_rejects_non_canonical_encoding() {
    assert!(
        ScalarField::from_hex(&ScalarField::N.to_hex()).is_err(),
        "hex parsing must reject non-canonical scalar encodings"
    );
}
...
fn scalar_distributivity() {
    ...
    assert_eq!(
        lhs.to_bytes_le(),
        rhs.to_bytes_le(),
        "distributivity a*(b+c) == a*b+a*c failed"
    );
}
...
#[test]
fn point_mul_matches_simple_for_small_scalars() {
    let generator = Point::generator();

    for scalar_value in 0u64..128 {
        let scalar = ScalarField::from_u64(scalar_value);
        let windowed = generator.mul(&scalar);
        let simple = generator.mul_simple(scalar_value);
        assert_eq!(
            windowed, simple,
            "windowed multiplication diverged from simple multiplication for scalar {scalar_value}"
        );
    }
}
...
fn scalar_ct_ne_different_values() {
    ...
    if a.to_bytes_le() != b.to_bytes_le() {
        assert_ne!(
            a, b,
            "two independently generated scalars must not be equal"
        );
    }
}
