...
use crate::{hash_no_pad, Goldilocks, HashOut};
...
impl MerkleTree {
    ...
    pub fn prove(&self, index: usize) -> Option<MerkleProof> {
        ...
        for layer in &self.layers[..self.layers.len().saturating_sub(1)] {
            ...
            siblings.push(layer.get(sibling_idx).copied().unwrap_or_else(zero_hash));
            idx /= 2;
        }
...
mod tests {
    ...
    fn prove_and_verify_all_leaves() {
        ...
        for (i, leaf_hash) in leaves.iter().enumerate() {
            ...
            assert!(
                MerkleTree::verify(root, &proof, *leaf_hash),
    ...
    fn non_power_of_two_leaves() {
        ...
        let root = tree.root();
        for (i, leaf_hash) in leaves.iter().enumerate().take(5) {
            let proof = tree.prove(i).unwrap();
            assert!(MerkleTree::verify(root, &proof, *leaf_hash));
        }
