...
This guide shows you how to run the included examples to learn how to use the Lighter Rust SDK.
...
2. **Environment file** - Create a `.env` file in the `lighter-rust` directory:
   ```bash
...
```bash
# From the lighter-rust directory
cd lighter-rust
...
1. Create a new file in `api-client/examples/`:
   ```rust
   // my_example.rs
...
**Solution:** 
- Ensure `.env` file exists in the `lighter-rust` directory
...
- Check you're in the `lighter-rust` directory
