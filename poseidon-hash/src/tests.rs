#[cfg(test)]
mod unit_tests {
    use crate::{Fp5Element, Goldilocks};
    ...
    fn test_goldilocks_field_edge_cases() {
        ...
        let expected = ((Goldilocks::ORDER - 1) as u128 * (Goldilocks::ORDER - 1) as u128)
            % Goldilocks::ORDER as u128;
    ...
    fn test_fp5_field_operations() {
        ...
        // Test zero and one
        assert!(Fp5Element::zero().is_zero());
        assert_eq!(Fp5Element::one(), Fp5Element::one());
        assert!(!Fp5Element::one().is_zero());
    }
    ...
    fn test_goldilocks_distributivity() {
        ...
        assert_eq!(
            lhs.to_canonical_u64(),
            rhs.to_canonical_u64(),
    ...
    #[test]
    fn test_goldilocks_try_inverse_of_zero_is_none() {
        assert!(
            Goldilocks::zero().try_inverse().is_none(),
            "zero should not report a multiplicative inverse"
        );
    }

    #[test]
    fn test_goldilocks_inverse_matches_try_inverse_for_non_zero_values() {
        let values = [1u64, 2, 5, 7, 12345, 999_999_999, Goldilocks::ORDER - 1];
        for v in values {
            let a = Goldilocks::from_canonical_u64(v);
            let strict = a
                .try_inverse()
                .expect("non-zero value must have an inverse");
            assert_eq!(strict, a.inverse(), "strict and legacy inverse must match");
        }
    }
    ...
    fn test_goldilocks_sqrt_of_perfect_squares() {
        for &v in &[1u64, 4, 9, 16, 25, 100, 12345678] {
            ...
            let sqrt = a_sq
                .sqrt()
                .expect("square of a positive value must have a sqrt");
            assert_eq!(
