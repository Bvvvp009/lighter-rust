use goldilocks_crypto::{Goldilocks, ScalarField};
use thiserror::Error;
    ...
    pub fn new(private_key_bytes: &[u8]) -> Result<Self> {
        if private_key_bytes.len() != 40 {
            return Err(SignerError::Crypto(
                goldilocks_crypto::CryptoError::InvalidPrivateKeyLength(private_key_bytes.len()),
            ));
        }
        // Use all 40 bytes for 5-limb scalar
        let private_key = ScalarField::from_bytes_le(private_key_bytes).map_err(|_| {
            SignerError::Crypto(goldilocks_crypto::CryptoError::InvalidPrivateKeyLength(
