...
mod tests {
    use crate::{
        empty_hash_out, hash_n_to_one, hash_no_pad, hash_to_quintic_extension, Fp5Element,
        Goldilocks,
    };
    ...
    fn hash_to_quintic_extension_is_deterministic() {
        ...
        assert_eq!(
            a.to_bytes_le(),
            b.to_bytes_le(),
            "same input must always give same hash"
        );
    }

    #[test]
    fn hash_no_pad_is_deterministic() {
        let input: Vec<Goldilocks> = (10u64..=15).map(Goldilocks::from_canonical_u64).collect();
    ...
    fn single_bit_flip_changes_hash_to_quintic_extension() {
        ...
        assert_ne!(
            hash_base.to_bytes_le(),
            hash_mod.to_bytes_le(),
    ...
    fn hash_of_all_zeros_is_not_zero() {
        ...
        assert!(
            !result.is_zero(),
            "hash of all-zero input should not be zero"
        );
    }
    ...
    #[test]
    fn fp5_try_inverse_of_zero_is_none() {
        let z = Fp5Element::zero();
        assert!(
            z.try_inverse().is_none(),
            "strict inverse API should reject zero instead of masking it"
        );
    }

    #[test]
    fn fp5_from_bytes_rejects_noncanonical_limb() {
        let mut bytes = [0u8; 40];
        bytes[..8].copy_from_slice(&Goldilocks::MODULUS.to_le_bytes());
        assert!(
            Fp5Element::from_bytes_le(&bytes).is_err(),
            "Fp5 bytes with a non-canonical Goldilocks limb must be rejected"
        );
    }

    #[test]
    fn fp5_try_inverse_roundtrip_for_deterministic_samples() {
        for i in 1u64..=32 {
            let x = Fp5Element::from_uint64_array([
                i,
                i.wrapping_mul(3),
                i.wrapping_mul(5),
                i.wrapping_mul(7),
            ...
            let inv = x.try_inverse().expect("non-zero sample must invert");
            assert_eq!(
                x.mul(&inv),
                Fp5Element::one(),
                "sample {i} must invert cleanly"
            );
        }
    }
